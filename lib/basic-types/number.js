...
(function() {
    ...
    this.checkSubsetOf = function(targetSchema){
      const isDivisibleSubsetOf = (() => { switch(true){
        case this[_divisibleBy] === 0: return targetSchema[_divisibleBy] === 0
        case targetSchema[_divisibleBy] === 0: return true
        default: return this[_divisibleBy] % targetSchema[_divisibleBy] === 0
      }})
      
      switch(true){
        case targetSchema instanceof NumberSchema !== true: return {isSubset: false, reason: `not comparing with another NumberSchema` }; 
        case !isDivisibleSubsetOf(): return {isSubset: false, reason: `source division check value = ${this[_divisibleBy]} is not divisible by target value = ${targetSchema[_divisibleBy]}` }; 
        case this[_max] > targetSchema[_max]: return {isSubset: false, reason: `target maximum value = ${targetSchema[_max]} is smaller than source value = ${this[_max]}` }; 
        case this[_min] < targetSchema[_min]: return {isSubset: false, reason: `target minimum value = ${targetSchema[_min]} is bigger than source value = ${this[_min]}` }; 
