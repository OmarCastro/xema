  ...
  instanceProperties: {
    ...
    get errors () {
      const errors = Object.keys(this[_keys])
        .filter(key => this[_keys][key] != null)
        .map(key => ({error: this[_keys][key].errors, key}))
        .filter(recordValidation => recordValidation.error != null && recordValidation.error.length > 0)
...
applyFeaturesOn(ObjectSchemaMaker, [OptionalValueFeature, GeneratorOptionCleanFeature, SchemaInstanceValidationFeature, SchemaValidationResultCacheFeature, BaseSubsetValidationFeature])
